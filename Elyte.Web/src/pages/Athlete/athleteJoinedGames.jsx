...
const AthleteJoinedGames = ({ athleteId }) => {
    const [games, setGames] = useState([]);

    useEffect(() => getAthleteGames(), [athleteId]);

    const getAthleteGames = async () => {
        const res = await athleteService.getAthleteJoinedGames(athleteId);
        if (res && res.code === 200) {
...
export default AthleteJoinedGames;