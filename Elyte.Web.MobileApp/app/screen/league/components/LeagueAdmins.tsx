...
const LeagueAdmins = ({ profile }) => {
    ...
    const getAdmins = async () => {
        const res: any = await leagueService.getLeagueAdmins(profile?.id);
        if (res && res.code === 200) {
    ...
    const getCoordinators = async () => {
        const res: any = await leagueService.getLeagueCoordinators(profile?.id || '');
    ...
    const handleRemoveAdmin = async () => {
        const res: any = await leagueService.cancelLeagueAdmin(profile.id, memberId);
    ...
    const handleRemoveCoordinator = async () => {
        const res: any = await leagueService.cancelLeagueCoordinator(profile.id, coordinatorId);
    ...
    return (
        <Box mt={2}>
            ...
            />      
            <ElConfirm
                title="Remove league admin"
                message="Are you sure to remove this league admin?"
                visible={showDeleteAdminConfirmDialog}
                onCancel={() => setShowDeleteAdminConfirmDialog(false)}
                onConfirm={handleRemoveAdmin}
            />
            <ElConfirm
                title="Remove league coordinator"
                message="Are you sure to remove this league coordinator?"
